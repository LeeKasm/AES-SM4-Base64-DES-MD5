...
int md5String(uint8_t* input_buffer, size_t input_size, uint8_t* output_buffer, size_t output_size, size_t* return_size) {
	...
	if (input_buffer == NULL || input_size == 0 || return_size == NULL)
		return 0;

	*return_size = 16;

	if (output_buffer == NULL || output_size == 0 || output_size < *return_size)
		return 0;
	...
	memcpy(output_buffer, ctx.digest, 16);
	return 1;
}

uint8_t* md5File(FILE *file){

	char* input_buffer = (uint8_t*)calloc(1, 1024);
	...
	uint8_t *result = (uint8_t*)calloc(1, 16);
